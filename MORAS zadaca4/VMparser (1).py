class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            ...
            if len(l) == 2:
                self._defined_labels.add(l[1])
                return "(" + l[1] + ")"

        elif l[0] in ["goto", "if-goto"]:
            if len(l) == 2:
                self._referenced_labels.add((l[1], n, l[0]))
                return "//" + " ".join(l) + "\n" + self._goto(l[0], l[1], n)
            else:
                self._flag = False
                Parser._error("Parser", n, "Invalid goto command.")
                return ""
    ...
    def _goto(self, command, label, n):
        if command == "goto":
            return "@" + label + "\n0;JMP"
    ...
    def _checkUndefinedLabels(self):
        undefined_labels = set()
        for label, _, _ in self._referenced_labels:
            if label not in self._defined_labels:
                undefined_labels.add(label)

        for label, line, command in self._referenced_labels:
            if label in undefined_labels:
                Parser._error("Label", line, "Undefined label '" + label + "' used in " + command + ".")
